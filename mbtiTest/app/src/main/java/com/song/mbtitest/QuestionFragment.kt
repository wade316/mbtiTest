...
class QuestionFragment : Fragment() {
    ...
    private val questioAnswers = listOf(
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        val answerRadioGroup = listOf<RadioGroup>(
            view.findViewById(R.id.rg_ansewer_1),
            view.findViewById(R.id.rg_ansewer_2),
            view.findViewById(R.id.rg_ansewer_3)
        )
        ...
        for (i in questionTextView.indices) {
            ...
            radioButton1.text = getString(questioAnswers[questionType][i][0])
            radioButton2.text = getString(questioAnswers[questionType][i][1])
        ...
        btnnext.setOnClickListener {
            ...
            if(isAllAnswered) {
                val responses = answerRadioGroup.map {radioGroup ->
                    val firstRadioGroupButton =  radioGroup.getChildAt(0) as RadioButton
                    if(firstRadioGroupButton.isChecked) 1 else 2
                }
                ...
                Toast.makeText(context, "모든 질문의 답해주세요.",Toast.LENGTH_SHORT).show()
            }
        }
        if (questionType==3){
            btnnext.setText("결과확인")
        }
    }
